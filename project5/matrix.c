...
TMatrix * cofactorMatrix(TMatrix *m, int p, int q, int n)
{
    TMatrix *cofactor;
    ...
    for(int row = 0; row < n; row++)
    {
        for(int col = 0; col < n; col++)
        {
            if(row != p && col != q)
            {
                cofactor->data[i][j++] = m->data[row][col];
                if(j == n - 1)
                {
                    j = 0;
                    i++;
...
double determinant(TMatrix *m, int n)
{
    int N = m->nrows;
    double sum = 0;
    if(n == 1)
        return m->data[0][0];
    TMatrix *temp = newMatrix(N, N);
    int sign = 1;
    for(int f = 0; f < n; f++)
    {
        temp = cofactorMatrix(m, 0, f, n);
        sum += sign * m->data[0][f] * determinant(temp, n - 1);
        sign = - sign;
    }
    return sum;
}

TMatrix * adjointMatrix(TMatrix *m)
{
    int n = m->nrows;
    TMatrix *adjoint, *temp;
    adjoint = newMatrix(n, n);
    if(n == 1)
    {
        adjoint->data[0][0] = 1;
        return adjoint;
    ...
    for(int i = 0; i < n; i++)
    {
        for(int j = 0; j < n; j++)
        {
            temp = cofactorMatrix(m, i, j, n);
            sign = ((i+j)%2==0)? 1: -1;
            adjoint->data[j][i] = sign * determinant(temp, n - 1);
        }
    }
    return adjoint;
}

TMatrix * invertMatrix(TMatrix *m)
{
    int n = m->nrows;
    double det = determinant(m, n);
    if(det == 0)
    {
        printf("SINGULAR ~ returning NULL");
        return NULL;
    } else
    {
        TMatrix *adjoint = adjointMatrix(m);
        TMatrix *inverse = newMatrix(n, n);
        for(int i = 0; i < n; i++) for(int j = 0; j < n; j++) inverse->data[i][j] = adjoint->data[i][j] / det;
        return inverse;
    }
}

// ENDOF UNSAFE METHODS
